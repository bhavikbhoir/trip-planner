...
- **Itinerary** (`Itinerary.jsx`): AI-generated day-by-day plan with real grounded data (opening hours, parking, driving/walking travel times between stops), manual bookings shown as timeline anchors, suggestions + per-member approval tracking, an AI advisor tips panel (hotel-area fit, arrival gaps, tight departures, coverage gaps — dismissible, one-click "turn into a suggestion"), restaurant alternatives per meal event (swap the AI's pick), and an owner-only "Edit trip" panel (`EditTripForm.jsx`) for name/destination/dates/type.
- **Day-of view** (`DayOf.jsx`): defaults to today, with Previous/Next-day navigation (and a "Today" jump-back) so you can catch up on a day you forgot to check off. Each event gets a 3-way Done / Skip / Did-something-else status (skip and swap take an optional note — mutually exclusive with each other and with done) for tracking what actually happened while the trip is underway.
- **Bookings** (`Bookings.jsx`): the shared management home for a trip's hotel/car/other bookings — any member can add, edit, or delete any of them. The Itinerary shows these same bookings as read-only plan anchors.
...
- **Recap** (`Recap.jsx`): the trip's closing view, reachable once a trip is marked complete — completion stats (done/skipped/swapped vs. planned), a "what we did differently" list pulled from skip/swap notes, the final settle-up, and a per-member mood + comment feedback form. "Mark trip complete" lives on `Itinerary.jsx`'s action bar (any member, no approval gate — see `src/utils/tripPhase.js` for how trip phase is derived); "Reopen trip" lives on Recap itself.
- **Settings** (`Settings.jsx`): update display name (syncs Cognito + every trip the user belongs to).
- **Navigation**: `TripTabs.jsx` — Itinerary / Today / Bookings / Expenses / Recap tab strip on every trip page (Recap only shown once a trip is completed; scrolls horizontally on narrow screens); `AppShell.jsx` — global shell with a polling notification bell (member joined, suggestion added, plan regenerated) and a Settings link.
...
the group suggests changes and approves, day-of Done/Skip/Swap tracking and
expense settle-up carry the trip through to the end, and a post-trip recap
(completion stats, what changed vs. plan, mood feedback) closes it out. A
starter unit test suite covers the pure logic; component-level testing is
the next real gap.
